...
export async function POST() {
  try {
    console.log('Exhibition 이미지 마이그레이션 시작...');
    ...
    for (const [exhibitionId, exhibition] of Object.entries(metadata)) {
      ...
      for (const photoKey of exhibition.photos) {
        try {
          ...
          if (photoKey.includes('/Original/') || photoKey.includes('/Thumbnail/')) {
            console.log(`${photoKey}: 이미 마이그레이션된 파일입니다.`);
          ...
          // 파일명 추출
          const filename = photoKey.split('/').pop() || `photo_${Date.now()}.jpg`;
          const originalKey = `Exhibitions/${exhibitionId}/Original/${filename}`;
          const thumbnailKey = `Exhibitions/${exhibitionId}/Thumbnail/${filename}`;

          // 썸네일 생성 (400x400px, 품질 85%)
          ...
          console.log(`마이그레이션 완료: ${photoKey} -> ${originalKey}, ${thumbnailKey}`);

          // 썸네일 키를 업데이트된 사진 목록에 추가
          updatedPhotos.push(thumbnailKey);
          ...
          console.log(`원본 파일 삭제 완료: ${photoKey}`);
    ...
    return NextResponse.json({
      success: true,
      message: 'Exhibition 이미지 마이그레이션 완료',
      totalProcessed,
      totalOptimized,
      results
    });
