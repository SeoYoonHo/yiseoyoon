...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Drawing Presigned URL 요청:', { fileName, fileType, title, year, description });
    ...
    // 썸네일 이미지 키들
    const thumbnailSmallKey = `Works/Drawing/Thumbnail/Small/${timestamp}.${fileExtension}`;
    const thumbnailMediumKey = `Works/Drawing/Thumbnail/Medium/${timestamp}.${fileExtension}`;
    const thumbnailLargeKey = `Works/Drawing/Thumbnail/Large/${timestamp}.${fileExtension}`;

    console.log('Drawing S3 키 생성:', { originalKey, thumbnailSmallKey, thumbnailMediumKey, thumbnailLargeKey });
    ...
    console.log('Drawing Presigned URLs 생성 완료');
